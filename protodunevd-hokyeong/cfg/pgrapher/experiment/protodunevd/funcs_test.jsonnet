...

{
    ...
    // local cru_halves = [
    //   std.range(0,475) + std.range(952,1427) + std.range(1904,2487),
    //   std.range(476,951) + std.range(1428,1903) + std.range(2488,3071),
    // ],

    // Full CRP listing used by channel DBs, Magnify, etc.
    anode_channels :: function(n) {
      local crp = (n - n%2)/2,
      ...
      // local channels = cru_halves[0] + cru_halves[1],
      ret: [x + 3072 * crp for x in channels],
    }.ret,

    // Half-CRP listing retained for configs that still operate per CRU.
    anode_cru_channels :: function(n) {
      local crp = (n - n%2)/2,
      local cru = n % 2,
      ret: [x + 3072 * crp for x in cru_halves[cru]],
    }.ret,
  ...
  splat:: function(params, tools, anode, name=null) {
    ...
    local sp = g.pnode({
        ...
        data: {
            ...
            // time_offsets: [-122.5*wc.us, -122.5*wc.us, -122.5*wc.us],
